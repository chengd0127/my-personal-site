...
export function Navbar({ profile, currentPath, site = { path: (value) => value } }) {
  const links = navItems
    .map((item) => {
      const isActive = item.href === currentPath;
      return `<a class="nav-link${isActive ? " active" : ""}" href="${escapeHtml(site.path(item.href))}"${
  ...
  return `
    <header class="site-header">
      <a class="brand" href="${escapeHtml(site.path("/"))}" aria-label="${escapeHtml(profile.name)} home">
